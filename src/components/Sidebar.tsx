...
function WebGuideTabs() {
  ...
  return <div className="flex min-w-0 flex-1 items-center justify-center gap-1 overflow-x-auto px-2" role="tablist" aria-label="Guias web">
    {tabs.map((tab) => { const active = activeTabId === tab.id; const google = tab.id === 'app-google'; return <div key={tab.id} role="tab" aria-selected={active} aria-controls="workspace-content" title={tab.title} className={`group flex h-8 max-w-56 min-w-0 items-center gap-1 rounded-md border px-2 text-xs ${active ? 'border-core-accent/60 bg-core-accent/10 text-white' : 'border-transparent text-slate-400 hover:bg-white/5'}`}>
...
export function GlobalTopBar() {
  ...
  return <aside data-testid="coredesk-topbar" data-debug="coredesk-topbar" className="relative z-[100] flex shrink-0 items-center gap-2 border-b border-core-line bg-core-panel px-2 text-slate-100" style={{ height: SHELL_LAYOUT.globalBarHeight }} onClick={() => menuProfileId && setMenuProfileId(undefined)}>
    <button onClick={() => selectTab('home')} className={`${moduleButton} ${activeTabId === 'home' ? 'bg-core-accent/10 text-core-accent' : ''}`} title="Início" aria-label="Início"><Home size={18} /></button>
    <div className="mx-1 h-6 w-px shrink-0 bg-core-line" />
    <div className="flex min-w-0 shrink-0 items-center gap-2 overflow-x-auto px-1">{profiles.map((profile) => { const active = activeTabId === `whatsapp:${profile.id}`; const status = runtime[profile.id]; return <button key={profile.id} title={`${profile.name} · ${profile.suspended ? 'Suspenso' : status?.connectionState ?? 'Sessão ativa'}`} aria-label={profile.name} onClick={(event) => { event.stopPropagation(); void (profile.suspended ? resumeProfile(profile.id) : openProfile(profile.id)) }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); setMenuProfileId(profile.id) }} className={`relative grid h-10 w-10 shrink-0 place-items-center rounded-md ${active ? 'bg-white/8 ring-1 ring-core-accent/50' : 'hover:bg-white/5'}`}>{active && <span className="absolute -bottom-1 left-2 h-0.5 w-6 rounded bg-core-accent" />}<ProfileAvatar profile={profile} size="sm" /><span className={`absolute bottom-1 right-1 h-2 w-2 rounded-full border border-core-panel ${profile.suspended ? 'bg-slate-500' : status?.connectionState === 'disconnected' ? 'bg-red-400' : status?.connectionState === 'loading' ? 'bg-amber-300' : 'bg-cyan-400'}`} />{Boolean(status?.unreadCount) && <span className="unread-badge absolute right-0 top-0 min-w-4 rounded-full bg-core-accent px-1 text-[8px] font-bold leading-4 text-slate-950">{formatUnreadCount(status.unreadCount)}</span>}</button> })}<button onClick={addProfile} className="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-dashed border-core-line text-slate-600 hover:border-core-accent/50 hover:text-core-accent" title="Adicionar perfil" aria-label="Adicionar perfil"><Plus size={16} /></button></div>
    <WebGuideTabs />
    <div className="mx-1 h-6 w-px shrink-0 bg-core-line" />
    <button onClick={openCommunication} className={`${moduleButton} ${activeTabId === 'communication' ? 'bg-core-accent/10 text-core-accent' : ''}`} title="Comunicação" aria-label="Comunicação"><MessageSquare size={18} /></button>
