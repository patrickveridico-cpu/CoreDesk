...
describe('TechRoute-compatible operations', () => {
  ...
  it('creates quote and emits route/quote events', async () => { const dir = await mkdtemp(path.join(os.tmpdir(), 'coredesk-ops-')); const events = new EventBus(); const seen: string[] = []; events.on('operations:quote-calculated', () => seen.push('quote')); const service = new OperationsService(dir, new StorageService(), events, new LoggerService(dir, 'test', false)); await service.initialize(); const base = await service.saveBase({ name: 'Base', address: 'Rua A' }); const insurer = await service.saveInsurer({ name: 'Seguradora' }); const specialty = await service.saveSpecialty({ name: 'Reboque Leve' }); await service.savePricingTable({ insurerId: insurer.id, specialtyId: specialty.id, exitValue: 100, kmFranchise: 40, kmValue: 2 }); const route = service.calculateRoute({ origin: 'A', destination: 'B', totalKm: 60 }); const result = await service.calculateQuote({ insurerId: insurer.id, specialtyId: specialty.id, baseId: base.id, origin: 'A', destination: 'B', route }); expect(result.calculation.total).toBe(140); expect(seen).toEqual(['quote']); await rm(dir, { recursive: true, force: true }) })
})
