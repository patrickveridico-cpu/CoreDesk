...
export interface OperationBase { id: string; name: string; address: string; city?: string; state?: string; latitude?: number; longitude?: number; specialties?: string[]; status: EntityStatus; notes?: string; legacyId?: string; createdAt?: string; updatedAt?: string; archivedAt?: string }
export interface OperationInsurer { id: string; name: string; company?: string; code?: string; status: EntityStatus; notes?: string; legacyId?: string; createdAt?: string; updatedAt?: string; archivedAt?: string }
export interface OperationSpecialty { id: string; name: string; category?: string; status: EntityStatus; legacyId?: string; createdAt?: string; updatedAt?: string; archivedAt?: string }
export interface PricingTable { id: string; company?: string; companyId?: string; insurerId: string; specialtyId: string; exitValue: number; kmFranchise: number; kmValue: number; workHourValue?: number; validFrom?: string; validUntil?: string; status: EntityStatus; legacyId?: string; legacyCompanyName?: string; migrationStatus?: string; migrationReason?: string; conflictStatus?: string; conflictGroupId?: string; version?: number; versionOf?: string; changeReason?: string; createdAt?: string; updatedAt?: string; archivedAt?: string }
...
export interface QuoteWorkspaceSnapshot {
  companyId?: string
  companyName?: string
  insurerName?: string
  ...
  routeText?: string
  source?: 'google-maps' | 'history'
}
export interface OperationQuote { id: string; code?: string; insurerId: string; specialtyId: string; baseId: string; origin: string; destination: string; stops: string[]; route: RouteCalculation; calculation: QuoteCalculation; financialSnapshot?: FinancialSnapshot; workspaceSnapshot?: QuoteWorkspaceSnapshot; message: string; originalMessage?: string; editedMessage?: string; status: 'draft' | 'saved' | 'cancelled' | 'archived'; revision?: number; revisionOf?: string; revisionReason?: string; legacyId?: string; observations?: string; createdAt: string; updatedAt: string }
